...
public class PreVoucher extends FinanceTrans implements TransPresenter {
    ...
    public void start() {
        ...
        if (setAmount()) {
            if (CardProcess() == 0) {
                transUI.trannSuccess(timeout, Tcode.Status.venta_success);
    ...
    public void isICC() {
        ...
        emv = new EmvTransaction(para, Type.PREVOUCHER);
        ...
        if (retVal == 0) {
            ...
            if (retVal == 0) {
                if (typeCoin != null) {
                    switch (typeCoin) {
                        case DOLAR:
                            transUI.trannSuccess(timeout, Tcode.Status.prevoucher_exitoso, "");
                            break;
                    }
                } else {
                    transUI.trannSuccess(timeout, Tcode.Status.prevoucher_exitoso, "");
