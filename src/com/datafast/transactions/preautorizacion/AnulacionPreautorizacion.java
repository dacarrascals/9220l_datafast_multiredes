...
public class AnulacionPreautorizacion extends FinanceTrans implements TransPresenter {

    public AnulacionPreautorizacion(Context ctx, String transEname, TransInputPara p) {
    ...
    private int procesarAnulacion() {

        if ((retVal = CommonFunctionalities.setNumReferencia(timeout, "ANULACION PREAUTORIZACION", transUI)) != 0) {
            return retVal;
        }
    ...
    private void isHandle() {
        if ((retVal = CommonFunctionalities.setPanManual(timeout, "ANULACION PREAUTORIZACION", transUI)) != 0) {
            return;
        }
        ...
        if ((retVal = CommonFunctionalities.setFechaExp(timeout, "ANULACION PREAUTORIZACION", transUI, ISOUtil.stringToBoolean(rango.getFECHA_EXP()))) != 0) {
            return;
        }

        ExpDate = CommonFunctionalities.getExpDate();

        /*if ((retVal = CommonFunctionalities.setCVV2(timeout, "ANULACION PREAUTORIZACION", transUI, ISOUtil.stringToBoolean(rango.getCVV2()))) != 0) {
            return;
        }
                ...
                if (typeCoin != null) {
                    switch (typeCoin) {
                        case DOLAR:
                            ...
                            transUI.trannSuccess(timeout, Tcode.Status.void_succ,"APROBADA #" + authCode);
                            break;
                    }
                } else {
                    transUI.trannSuccess(timeout, Tcode.Status.void_succ, "");
