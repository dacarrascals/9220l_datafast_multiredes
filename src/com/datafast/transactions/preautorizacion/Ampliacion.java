...
public class Ampliacion extends FinanceTrans implements TransPresenter {

    public Ampliacion(Context ctx, String transEname, TransInputPara p) {
    ...
    public void start() {
        ...
        if (procesarAmpliacion() == 0) {
            transUI.trannSuccess(timeout, Tcode.Status.ampliacion);
        ...
        Logger.debug("ampliacionTrans>>finish");

    }

    private int procesarAmpliacion() {
        if (setAmount()) {
            retVal = CardProcess();
        }
        return retVal;
                ...
                if (typeCoin != null) {
                    switch (typeCoin) {
                        case DOLAR:
                            ...
                            transUI.trannSuccess(timeout, Tcode.Status.ampliacion,"APROBADA #" + authCode);
                            break;
                    }
                } else {
                    transUI.trannSuccess(timeout, Tcode.Status.ampliacion,"");
