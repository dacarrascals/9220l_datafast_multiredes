...
public class Confirmacion extends FinanceTrans implements TransPresenter {

    public Confirmacion(Context ctx, String transEname, TransInputPara p) {
    ...
    public void start() {
        ...
        if (procesarConfirmacion() == 0) {
            transUI.trannSuccess(timeout, Tcode.Status.confirmacion);
        ...
        Logger.debug("ConfirmacionTrans>>finish");
    }

    private int procesarConfirmacion() {
        if (setAmount()) {
            retVal = CardProcess();
        }
        return retVal;
    ...
    private void prepareOnline() {
        ...
        if ((retVal = CommonFunctionalities.setIdPreAutoAmpliacion(timeout, TransEName, transUI)) != 0) {
            retVal = Tcode.T_user_cancel_input;
            return;
        }
                ...
                if (typeCoin != null) {
                    switch (typeCoin) {
                        case DOLAR:
                            ...
                            transUI.trannSuccess(timeout, Tcode.Status.confirmacion, "APROBADA #" + authCode);
                            break;
                    }
                } else {
                    transUI.trannSuccess(timeout, Tcode.Status.confirmacion, "");
