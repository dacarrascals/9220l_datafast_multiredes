...
public class PreAutorizacion extends FinanceTrans implements TransPresenter {

    public PreAutorizacion(Context ctx, String transEname, TransInputPara p) {
    ...
    public void start() {
        ...
        if (procesarPreAutorizacion() == 0) {
            transUI.trannSuccess(timeout, Tcode.Status.preauto_exitosa);
        ...
        Logger.debug("PreAutorizacionTrans>>finish");

    }

    private int procesarPreAutorizacion() {
        if (setAmount()) {
                ...
                if (typeCoin != null) {
                    switch (typeCoin) {
                        case DOLAR:
                            ...
                            transUI.trannSuccess(timeout, Tcode.Status.pre_auth_success,"APROBADA #" + authCode);
                            break;
                    }
                } else {
                    transUI.trannSuccess(timeout, Tcode.Status.pre_auth_success, "");
                }
            }
