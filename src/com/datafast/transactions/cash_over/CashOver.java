package com.datafast.transactions.cash_over;
...
public class CashOver extends FinanceTrans implements TransPresenter {

    public CashOver(Context ctx, String transEname, TransInputPara p) {
    ...
    public void start() {
        ...
        if (setAmount()) {
            if (CardProcess() == 0) {
                transUI.trannSuccess(timeout, Tcode.Status.cash_over_succ);
        ...
        Logger.debug("CashOverTrans>>finish");
    }
    ...
    private boolean setAmount() {
        ...
        if (amount.setAmount()) {
            ...
            CashOverAmount = amount.getmCashOverAmount();
            ...
            para.setAmountCashOver(CashOverAmount);
    ...
    public void isICC() {
        ...
        emv = new EmvTransaction(para, Type.CASH_OVER);
    ...
    private void prepareOnline() {
        ...
        if (retVal == 0) {
            ...
            Logger.debug("CashOverTrans>>OnlineTrans=" + retVal);
            clearPan();
            if (retVal == 0) {
                if (typeCoin != null) {
                    switch (typeCoin) {
                        case DOLAR:
                            ...
                            transUI.trannSuccess(timeout, Tcode.Status.cash_over_succ,"APROBADA #" + authCode);
                            break;
                    }
                } else {
                    transUI.trannSuccess(timeout, Tcode.Status.cash_over_succ, "");
